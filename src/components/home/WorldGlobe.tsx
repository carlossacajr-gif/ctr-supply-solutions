...
export function WorldGlobe({ showLabel = true }: WorldGlobeProps = {}) {
    ...
    return (
        <div className="relative flex items-center justify-center w-full max-w-2xl mx-auto pb-16">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[380px] h-[380px] sm:w-[650px] sm:h-[650px] rounded-full border border-slate-200/50 dark:border-slate-700/50 animate-[spin_30s_linear_reverse_infinite] border-dotted pointer-events-none" />
            ...
            {/* Live Indicator Pill */}
            {showLabel && (
                <div className="absolute bottom-6 sm:bottom-0 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full bg-white dark:bg-slate-950 shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-slate-100 dark:border-slate-800 z-20 flex items-center gap-3">
                    ...
                    <p className="text-xs text-slate-600 dark:text-slate-400 font-bold tracking-widest uppercase mb-0">Live Logistics Network</p>
                </div>
            )}
        </div>
    );
